...
* **역할**: 관리자 인증 비밀번호의 SHA-256 해시값 및 업데이트 타임스탬프 저장.
```json
{
...
```
* **초기 비밀번호**: `0000` (SHA-256 자동 셋업). 관리자 대시보드에서 변경 가능.
...
### (1) 관리자 인증 및 보안 체계
* **보안 저장소**: 루트 디렉터리의 `/admin.json`에 `passwordHash` (Web Crypto SHA-256) 형태로 안전하게 저장됩니다.
* **초기 비밀번호**: `0000`으로 자동 설정되며, 관리자 대시보드 [보안 설정] 탭에서 언제든지 새 비밀번호로 갱신 가능합니다.
