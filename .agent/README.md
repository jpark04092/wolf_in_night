...
* **핵심 컨셉**:
  ...
  8. **게임 종료 후 연속 재시작(리매치) 및 원자적 데이터 정리**: 한 게임이 끝난 후 방장이 대기실로 돌아가거나 다음 게임을 시작할 때, 이전 게임의 투표 파일(`votes/`) 및 유저 시작 직업(`initialRole`)을 원자적으로 정리하고 세션 핑거프린트(`stepStartedAt`)로 새 게임을 식별하여 모든 플레이어에게 새 직업 확인 모달과 정상적인 밤 액션이 제공되도록 보장.
