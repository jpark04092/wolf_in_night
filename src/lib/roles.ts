...
// Default role pool based on player count (Always Players + 3 center cards)
export function generateDefaultDeck(playerCount: number): RoleType[] {
  const totalCards = playerCount + 3;
  // Core classic setup:
  // 2 Werewolves, 1 Seer, 1 Robber, 1 Troublemaker, 1 Insomniac, Villagers to fill
  ...
  const shuffled = [...deck.slice(0, totalCards)];
